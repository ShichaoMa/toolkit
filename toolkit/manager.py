...
class Blocker(object):
    ...
    eg
    def myfunc(self):
        with SleepManager(sleep_time, self, interval=1, notify=lambda x: not x.alive) as sm:
            if sm.is_notified:
                `我们知道是被唤醒了，而不是时间到了`
            ....
    """
    def __init__(self, block_time, instance, interval=1, notify=lambda instance: False, immediately=False):
        """
    ...
    def __enter__(self):
        while not self.immediately and time.time() - self.enter_time < self.block_time and not self.notify(self.instance):
